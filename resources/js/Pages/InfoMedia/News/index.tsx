...
export default function Page(props: ViewPaginateGeneric<News>) {
  const { t } = useTranslation();
  const t_opt = { ns: 'infoMedia' };
  const data = props.data.data;
  const pageRoute = 'info-media.berita.index';

  const dataMapped = data.map((data) => {
    console.log(data);
    return (
      <Link href={route('info-media.berita.show', { slug: data.slug })}>
        <NormalCard
          title={data.judul ?? ''}
  ...
  return (
    <>
      <AppMeta title="Berita" />
      <PublicLayout {...props}>
        <PaginatedCardLayout
          breadCrumb={
            <CustomBreadcrumb
              items={[{ label: 'Beranda', href: '/' }, { label: 'Info & Media' }, { label: 'Berita', href: '#' }]}
            />
          }
          title={t('news', t_opt)}
          data={dataMapped}
